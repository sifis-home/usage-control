...
import ucs.constants.STATUS;
import ucs.sessionmanager.SessionAttributes;
import ucs.exceptions.PolicyException;
import ucs.exceptions.RequestException;
import ucs.exceptions.StatusException;
...
import ucs.sessionmanager.OnGoingAttributesInterface;
import ucs.sessionmanager.SessionAttributesBuilder;
import ucs.sessionmanager.SessionInterface;
import utility.errorhandling.Reject;
import xacml.Attribute;
import xacml.Category;
import xacml.PolicyTags;
...
public final class ContextHandler extends AbstractContextHandler {
    ...
    public TryAccessResponseMessage tryAccess( TryAccessMessage message ) throws PolicyException, RequestException {
        ...
        log.log( Level.INFO, "TryAccess evaluated at "+System.currentTimeMillis()+" pdp response : " +
        ...
        log.severe("TRY ACCESS completed, evaluation: " + evaluation.getResult() + ", sessionId: " + sessionId);
    ...
    private TryAccessResponseMessage buildTryAccessResponse( TryAccessMessage message, PDPEvaluation evaluation, String sessionId ) {
        TryAccessResponseMessage response = new TryAccessResponseMessage( message.getDestination(), message.getSource(), message.getMessageId() );
    ...
    private void createSession( TryAccessMessage message, RequestWrapper request, PolicyWrapper policy, String sessionId ) {
        log.log( Level.INFO, "Creating a new session, id: " +  sessionId );
        String pepUri = message.getSource() + PEP_ID_SEPARATOR + message.getDestination();
        ...
        SessionAttributes sessionAttributes = sessionAttributeBuilder.build();

        // insert all the values inside the session manager
        if( !getSessionManager().createEntry( sessionAttributes ) ) {
    ...
    public StartAccessResponseMessage startAccess( StartAccessMessage message )
        ...
        Reject.ifNull( evaluation );
        log.log( Level.INFO, "StartAccess evaluated at "+System.currentTimeMillis()+" pdp response : "+ evaluation.getResult());
        getObligationManager().translateObligations( evaluation, message.getSessionId(), STATUS.TRY );

        if( evaluation.isDecision( DecisionType.Permit ) ) {
            log.info("Permit Branch, updating entry");
    ...
    private StartAccessResponseMessage buildStartAccessResponse( StartAccessMessage message, PDPEvaluation evaluation ) {
        log.info("BuildStartAccessResponse");
    ...
    public synchronized void reevaluate( SessionInterface session ) throws PolicyException, RequestException {
        ...
        log.log( Level.INFO, "Reevaluate evaluated at "+System.currentTimeMillis()+" pdp response : " + evaluation.getResult());
    ...
    private ReevaluationResponseMessage buildReevaluationResponse( SessionInterface session, PDPEvaluation evaluation ) {
        ...
        log.info("session.getPepId() not splitted: " + session.getPepId());
        log.info("session.getPepId(): " + session.getPepId());
