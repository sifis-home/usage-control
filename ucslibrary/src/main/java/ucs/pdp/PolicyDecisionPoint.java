...
public final class PolicyDecisionPoint extends AbstractPDP {
    ...
    private ResponseType getResponseType( String response ) throws JAXBException {
        return XMLUtility.unmarshalToObject( ResponseType.class, response );
    }
    ...
    private ResponseCtx evaluate( String request, PolicyFinder policyFinder ) {
        try {
            // TODO UCS-36 NOSONAR
            RequestCtxFactory requestCtxFactory = RequestCtxFactory.getFactory();
            String requestReplaced = request.replaceAll( ">\\s+<", "><" );
            AbstractRequestCtx requestCtx = requestCtxFactory.getRequestCtx( requestReplaced );
    ...
    private AbstractResult evaluateContext( EvaluationCtx context, PolicyFinder policyFinder ) {
        Set<Attributes> attributesList = context.getRequestCtx().getAttributesSet();
        for(Attributes attributesElem: attributesList){
            Set<Attribute> attributes = attributesElem.getAttributes();
        }
