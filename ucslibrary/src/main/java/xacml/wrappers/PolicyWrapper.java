...
import wd_17.ApplyType;
import wd_17.AttributeDesignatorType;
import wd_17.AttributeValueType;
import wd_17.ConditionType;
import wd_17.EffectType;
import wd_17.PolicyType;
import wd_17.RuleType;
...
//                    log.info("Current condition decision time: " +conditionType.getDecisionTime());
        ...
        log.log( Level.WARNING, "Condition not found : " +  conditionName );
        return new ArrayList<>();
    }

    private List<Attribute> getAttributesFromCondition(ConditionType conditionType, String conditionName ) {
//        log.info("getAttributesFromCondition, conditionType: " + conditionType.getDecisionTime() + ", conditionName: " + conditionName);
...
//        log.info("getAttributesFromCondition, conditionType: " + conditionType);
//        log.info("conditionType.getExpression(): " + conditionType.getExpression());
        ArrayList<Object> elementList = new ArrayList<>();
        elementList.add(conditionType.getExpression());
        ArrayList<Attribute> attributeList = new ArrayList<>();
        int lastIndex = 0;
        for( int i = 0; i < elementList.size(); i++ ) {
            Object objValue = elementList.get( i );
            if( objValue instanceof ApplyType ) {
//                log.info("Is applyType");
                ApplyType applyType = (ApplyType) objValue;
//                log.info("expressions size: " + applyType.getExpression().size());
                elementList.addAll(applyType.getExpression());
                elementList.add(applyType.getAttributeDesignatorType());
                elementList.add(applyType.getAttributeValueType());
...
//                log.info("Is AttributeValueType");
...
//                    log.info("Content: " + obj.toString());
    ...
    private static PolicyType unmarshalPolicyType( String policy ){
        return XMLUtility.unmarshalToObject( PolicyType.class, policy );
    }

    public static String marshalPolicyType(PolicyType policy){
        return XMLUtility.marshalToString( PolicyType.class, policy, PolicyTags.POLICY, JAXBUtility.SCHEMA );
    }
